...
function Handler(params = {}) {
  ...
  config.smsTemplate = config.smsTemplate || "Please use the code - ${otp} to verify your phone for app authentication";
  ...
  // const producer = initProducerKafka({kafkaAddress: config.kafkaServer, password: config.kafkaPassword, username: config.kafkaUsername});
  // if (producer) {
  //   producer.connect()
  // }
  config.internalPatternEmail = /^[a-zA-Z0-9_.+-]+@thebank\.vn$/;
  config.projection = [];
  ...
  const serviceSelector = chores.newServiceSelector({
    serviceResolver,
    sandboxRegistry
  });
  ...
  function initProducerKafka({kafkaAddress, username, password}) {
    if (kafkaAddress) {
      const kafka = new Kafka({
        clientId: 'gkp-kafka-producer',
  ...
  async function kafkaSendMessage(config, producer, email) {
    if (producer) {
      const topic = 'backoffice.user.created';
      const messageObject = {from: config.emailServiceFrom,  to: email, type: "email", 
      template: config.emailTemplateId, templateData: { Weblink: config.emailBackOfficeLink }  };
  ...
  function registerAdminAccount(packet) {
    return Promise.resolve(packet).then(attachServices).then(loginAdminApp).then(detachServices);
  }
  function registerbackofficeAccount(packet) {
    return Promise.resolve(packet).then(attachServices).then(loginbackofficeApp).then(detachServices);
  }
  function registerClientAccount(packet) {
    return Promise.resolve(packet).then(attachServices).then(loginClientApp).then(detachServices);
  }
  function registerAgentAccount(packet) {
    return Promise.resolve(packet).then(attachServices).then(upsertDevice).then(validateUser).then(generateOTP).then(sendOTP).then(registerEnd).then(detachServices);
  }
  function registerCustomerAccount(packet) {
    return Promise.resolve(packet).then(attachServices).then(upsertDevice).then(validateCustomer).then(generateOTP).then(sendOTP).then(registerEnd).then(detachServices);
  }
  this.verificationCode = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(verifyOTP).then(detachServices);
  };
  this.refreshToken = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(refreshToken).then(detachServices);
  };
  this.revokeToken = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(revokeToken).then(detachServices);
  };
  this.updateUser = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(updateUser).then(filterUserInfo).then(detachServices);
  };
  this.getVerification = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(getVerification).then(detachServices);
  };
  this.resetVerification = function (packet, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(resetVerification).then(detachServices);
  };
  this.createUser = async function (packet = {}, options) {
    try {
      lodash.assign(packet, ctx)
      const {
        schemaManager,
        data
      } = packet;
      const { appType } = await validateAppType(injectOptions(packet, options))
      if (!data["email"]) {
        throw errorBuilder.newError("BackofficeAppEmailExpected", {
          payload: lodash.pick(data, ["email"])
        });
      }
      data.email = data.email.toLowerCase();
      if(!config.internalPatternEmail.test(data["email"])){
        throw errorBuilder.newError("EmailIsNotInternal");
      }
      let p = await getModelMethodPromise(schemaManager, "UserModel", "findOne");
      // query an user by the holderId
      const conditions = {};
      conditions[[appType, "email"].join(".")] = data["email"];
      const user = await p(conditions, null, {});
      if (user) {
        throw errorBuilder.newError("UserIsExisted", {
          payload: lodash.pick(data, ["email"])
        });
      } else {
        const user = {};
        assignUserData(appType, user, data);
        const userCreate = await getModelMethodPromise(schemaManager, "UserModel", "create");
        const opts = {};
        const userInfo = await userCreate([user],opts)
        // await kafkaSendMessage(ctx.config, ctx.producer, data["email"]);
  ...
  this.deleteUser = async function (packet = {}, options) {
    return validateAppType(injectOptions(packet, options)).then(attachServices).then(deleteUser).then(detachServices);
  };  
}
...
function getModelMethodPromise(schemaManager, modelName, methodName) {
  ...
  return Promise.resolve(Promise.promisify(model[methodName], {
    context: model
  }));
}
function loginAdminApp(packet = {}) {
  ...
  return Promise.resolve().then(function () {
    ...
    const conditions = {};
    conditions[[appType, "username"].join(".")] = {
    ...
    return bcryptor.compare(data.password, encPasswd).then(async function (matched) {
      if (matched) {
        ...
        const findOneAndUpdate = await getModelMethodPromise(schemaManager, "UserModel", "findOneAndUpdate");
        user = await findOneAndUpdate({
          _id: user._id
        }, {
          $inc: {
            [`${appType}.invalidPasswordCount`]: 1
          }
        }, {
          new: true
        });
...
function loginbackofficeApp(packet = {}) {
  ...
  return Promise.resolve().then(function () {
    ...
    if (!config.internalPatternEmail.test(userEmail)) {
...
function loginClientApp(packet = {}) {
  ...
  return Promise.resolve().then(function () {
    ...
    if (encTokenSecret === null) {
      return Promise.reject(errorBuilder.newError("TokenSecretNotFound", {
        payload: {
          appType: appType,
    ...
    return bcryptor.compare(data.tokenSecret, encTokenSecret).then(function (matched) {
      if (matched) {
        ...
        return Promise.reject(errorBuilder.newError("TokenSecretIsMismatched", {
          payload: {
            appType: appType,
            tokenKey: data.tokenKey
          },
...
function _findUserByPhoneNumber({
  schemaManager,
  config,
  appType,
  data,
...
function validateUser(packet = {}) {
  ...
  return _findUserByPhoneNumber(packet).then(function (user) {
  ...
  return Promise.resolve().then(function () {
    ...
    if (verification) {
      if (verification.expiredTime) {
        ...
        if (packet.hasResend) {
          const verificationFindOneAndUpdate = await getModelMethodPromise(schemaManager, "VerificationModel", "findOneAndUpdate");
...
async function sendOTP(packet = {}) {
  ...
  if (ref.service && ref.method) {
    ...
    const msgInfo = {
      ...
      text: format(config.smsTemplate, {
        otp: verification.otp
      })
    };
...
function verifyOTP(packet = {}) {
  ...
  return Promise.resolve().then(function () {
    ...
    return getModelMethodPromise(schemaManager, "UserModel", "findById").then(function (method) {
    ...
    const accessToken = oauthApi.createAppAccessToken({
      user,
      constraints: lodash.merge(lodash.pick(verification, ["appType", "expiredIn", "expiredTime", "phoneNumber"]), {
        email: user[verification.appType].email,
        permissionGroups: user[verification.appType].permissionGroups
      })
    });
    if (user && lodash.isFunction(user.toJSON)) {
      user = user.toJSON();
...
function refreshToken(packet = {}) {
  ...
  return Promise.resolve().then(function () {
    ...
    // if (appType === APP_TYPES.BACK_OFFICE) {
    //   const tokenParts = user[appType].googleToken.split('.');
    ...
    //     return Promise.reject(errorBuilder.newError('GoogleTokenIsExpired'));
    //   }
    // }
    let expiredIn = config.tokenExpiredIn;
    let expiredTime = momentHelper.getExpiredTime(expiredIn);
    // const now = moment();
...
function filterUserInfo(packet = {}) {
  ...
  if (appType === APP_TYPES.ADMIN) {
    data = lodash.assign(data, lodash.pick(lodash.get(user, appType), ["holderId", "username", "permissions", "permissionGroups"]));
  } else if (appType === APP_TYPES.CLIENT) {
    data = lodash.assign(data, lodash.pick(lodash.get(user, appType), ["holderId", "tokenKey", "email", "permissions"]));
  } else if (appType === APP_TYPES.AGENT) {
    data = lodash.assign(data, lodash.pick(lodash.get(user, appType), ["holderId", "phoneNumber"]));
  } else if (appType === APP_TYPES.CUSTOMER) {
    data = lodash.assign(data, lodash.pick(lodash.get(user, appType), ["holderId", "phoneNumber"]));
  }
  return lodash.assign(packet, {
...
function updateUser(packet = {}) {
  ...
  if (appType === APP_TYPES.ADMIN) {
    ...
    p = p.then(function (method) {
      ...
      // make the query
      return Promise.all([findByHolderId, findByUsername]).spread(function (byHolderId, byUsername) {
        if (data["password"]) {
    ...
    p = p.then(function (method) {
      ...
      // make the query
      return Promise.all([findByHolderId, findByPhoneNumber]).spread(function (userById, user) {
        if (userById) {
          if (user) {
    ...
    if (!data["holderId"] && !data["tokenKey"]) {
      return Promise.reject(errorBuilder.newError("ClientAppHolderIdOrTokenKeyExpected", {
    ...
    p = p.then(function (method) {
      ...
      // make the query
      return Promise.all([findByHolderId, findByTokenKey]).spread(function (byHolderId, byTokenKey) {
        if (byHolderId) {
          if (byTokenKey) {
...
function getVerification(packet = {}) {
  ...
  p = p.then(function (user) {
    ...
    return generateOTP(lodash.assign(packet, {
      user,
      device: lodash.pick(user, [appType, "device"])
    }));
  }).then(function ({
    verification
  }) {
...
function deleteUser(packet = {}) {
  ...
  let p = getModelMethodPromise(schemaManager, "UserModel", "findOne");
  if (data.holderId && (appType === APP_TYPES.AGENT || appType === APP_TYPES.CUSTOMER)) {
    p = p.then(function (method) {
      const conditions = {};
      conditions[[appType, "holderId"].join(".")] = data.holderId;
      return method(conditions, null, {});
    ...
    p = p.then(async function (user) {
      if (user) {
        user.agentApp.deleted = true;
        return user.save();
      }
      return Promise.resolve({});
    });
...
function matchFixedOTP(packet = {}, phoneNumber) {
  ...
  if (lodash.isArray(config.presetOTPs) && !lodash.isEmpty(config.presetOTPs)) {
    ...
    return getModelMethodPromise(schemaManager, "FixedotpModel", "findOne").then(function (method) {
      const conditions = {
...
function _extractUserQuery(appType, data) {
  return lodash.assign({
    appType
  }, lodash.pick(data, ["holderId", "phoneNumber", "username", "email"]));
}
function _checkUser({
  appType,
  errorBuilder,
  ...
  if (user[appType].deleted == true) {
    return Promise.reject(errorBuilder.newError("UserRequestDeleted", {
      payload: _extractUserQuery(appType, data),
      language
    }));
  }
